...
CheckLogin();
OpenDB();
function GenHtmURL($id){ return WEB_ROOT.'category/sort'.$id.'.htm';}
?><html>
<head>
...
<tr bgcolor="#F7F7F7"> 
  <td height="20" colspan="3" width="100%" background="images/topbg.gif" bgcolor="#F7F7F7">
  	  <table border=0 width="100%"><tr>
  	  	<td width="65%" nowrap><img src="images/pic5.gif" width="28" height="22" align="absmiddle" /><b>您现在所在的位置是： <a href="admincenter.php">管理首页</a> -&gt; <a href="mg_htmupdate.php">前台页面更新管理</a> -&gt; <font color=#FF0000>商品功能分类静态页面更新</font></b></td>
  	  	<td width="35%" nowrap align="right"><a href="#" onclick="ControlUpdate(true)"><b>开始自动更新</b></a>&nbsp;|&nbsp;<a href="#" onclick="ControlUpdate(false)"><b>停止自动更新</b></a></td>
  	  </tr></table>
  </td>
</tr><?php
$SortIndex=0;
...
function do_sort($selec,$index){
  ...
  foreach($res as $row){ 
    ...
    else echo '<tr bgcolor="#FFFFFF" height=25 onMouseOut="mOut(this)" onMouseOver="mOvr(this)"><td><a href="'.GenHtmURL($row['id']).'" target="_blank">'.str_repeat('　',$index*2).$SortIndex.'.&nbsp;'.$row['title'].'</a></td><td align="center">&nbsp</td><td align="center"><input type="button" value="更新" onclick="UpdateItem(this,\'id='.$row['id'].'\')"></td></tr>';
    do_sort($row['id'],$index+1);
  }
}?>
</table>
<script>InitHtmlUpdate(1,'mg_htmgen.php?mode=category','mytable');
</script>
</body>
</html><?php CloseDB();?>
