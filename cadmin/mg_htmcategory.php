...
function do_sort($selec,$index){
  ...
  $res=$conn->query('select * from mg_category where pid = '.$selec.' and recommend>0 order by sequence desc',PDO::FETCH_ASSOC);
  foreach($res as $row){
    $SortIndex++;
...
<script>InitHtmlUpdate(1,"mg_htmgen.php?mode=category","mytable");</script>
</body>
</html><?php db_close();?>
