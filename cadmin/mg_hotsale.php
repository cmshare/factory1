...
<table width="100%" height="100%" border="5" align="center" cellpadding="5" cellspacing="5" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
  <tr><form name="cartform" method="post" action="?"> 
    <td height=25 background="images/topbg.gif">
    	...
    	<tr>
    		<td nowrap>
    	     <b><img src="images/pic5.gif" width="28" height="22" align="absmiddle" />您现在所在的位置是： <a href="admincenter.php">管理首页</a> -&gt; <font color=#FF0000>商品销售排名</font></b>
...
$res=page_query('SELECT a.id, a.name,a.stock0,a.solded,sum(b.amount) as salenum','from ((mg_product as a inner join mg_ordergoods as b on a.id=b.productid) inner join mg_orders as c on c.ordername=b.ordername)','WHERE c.actiontime>'.$BeginDate.' and c.actiontime<'.$EndDate.' and  c.state>3 GROUP BY a.id, a.name,a.stock0,a.solded','order by salenum desc,a.solded desc',25);

if($total_records==0)echo '<br><br><p align=center>对不起，找不到相关记录！</p>';
else{
  echo '<table width="98%" border="2" align="center" cellpadding="1" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#CCCCCC">
        <tr bgcolor="#F7F7F7" align="center"> 
          ...
          <td width="10%" height="25" background="images/topbg.gif" bgcolor="#F7F7F7"><strong>商品编号</strong></td>
          <td width="55" background="images/topbg.gif"><strong>商品名称</strong></td>
          <td width="10%" background="images/topbg.gif" nowrap title="是指当前时间统计范围内售出的商品数量"><strong>本期售出(个)</strong></td>
          <td width="10%" background="images/topbg.gif" nowrap title="是指历史上累计售出的商品数量"><strong>累计售出(个)</strong></td>
          <td width="10%" background="images/topbg.gif" nowrap title="各子库存统计总和"><strong>库存统计</strong></td>
        </tr>';
  foreach($res as $row){
    echo '<tr height="25" align="center" bgcolor="#FFFFFF" onMouseOut="mOut(this)" onMouseOver="mOvr(this)">
        <td><input name="selectid[]" type="checkbox" value="'.$row['id'].'" onclick="mChk(this)"></td> <td><a href="mg_stocklog.php?id='.$row['id'].'">'.GenProductCode($row['id']).'</a></td>       
  ...
  echo '</table>
    ...
    <TR>
       ...
       <TD align="center"> 
        <script language="javascript">'."GeneratePageGuider(\"begindate=$str_begindate&enddate=$str_enddate\",$total_records,$page,$total_pages);".'</script>   	
       </TD>
    </tr>
    </TABLE>';
}?>
