...
function do_sort($parentid,$selec,$index){
  $res=$GLOBALS['conn']->query('select * from mg_category where parent = '.$selec.' order by sequence',PDO::FETCH_ASSOC);
  foreach($res as $row){
...
function add_save($parentid){
  ...
  if($title && is_numeric($sequence) && is_numeric($parentid) && $parentid>=0){
    $sql="mg_category set title='$title',sequence=$sequence,parent=$parentid,recommend=$recommend,isbrand=$isbrand,shared=0,description='$description'";
...
function edit_save(){ 
  ...
  $recommend=$_POST['recommend'];
  $description=FilterText(trim($_POST['description']));
  if($title && is_numeric($isbrand) && is_numeric($sequence) && is_numeric($parent) && is_numeric($brandid) && $brandid>0 && is_numeric($recommend)){
    if($brandid==$parent) PageReturn('父类不能指向自己！');
    $sql="update mg_category set title='$title',sequence=$sequence,parent=$parent,recommend=$recommend,isbrand=$isbrand,description='$description' where id=$brandid";
