...
if($mode=='sethot'){
   ...
   if(is_numeric($brandid) && $brandid>0 && is_numeric($recommend) && $recommend>1){
     if(!is_numeric($recommend)||$recommend<2)$recommend=2;
     if($conn->exec('update mg_category set recommend='.$recommend.' where recommend<=1 and id='.$brandid)) PageReturn('设置成功！');	  
   }
   PageReturn('操作失败！');
}
  ...
  if(is_numeric($brandid) && $brandid>0){
     if($conn->exec('update mg_category set recommend=1 where recommend>1 and id='.$brandid)) PageReturn('操作成功！');
  }
  PageReturn('操作失败！');
}
  ...
  <tr bgcolor="#F7F7F7"> 
    <td height="20" colspan="2" background="images/topbg.gif" bgcolor="#F7F7F7">
       <table width="100%" border="0" cellpadding="0" cellspacing="0">    	
    	  <tr><td width="50%"><b><img src="images/pic5.gif" width="28" height="22" align="absmiddle" />您现在所在的位置是： <a href="admincenter.php">管理首页</a> -&gt; <a href="mg_category.php">商品分类管理</a> -&gt; <font color=#FF0000>热销品牌设置</font></b></td>
    	      <td align="right"></td>
    	  </tr>
    	  </table>
    </td>
...
foreach($res as $row){
  ...
  <tr align="center" bgcolor="#FFFFFF" height="20" onMouseOut=mOut(this,"#FFFFFF") onMouseOver=mOvr(this,MENU_HOTTRACK_COLOR)> 
       <td height="25"><?php echo ++$sequence;?><input type="hidden" name="brandid" value="<?php echo $row['id'];?>"></td>
     ...
     <tr align="center" bgcolor="#FFFFFF" height="20" onMouseOut=mOut(this,"#FFFFFF") onMouseOver=mOvr(this,MENU_HOTTRACK_COLOR)> 
        <td height="25" background="images/topbg.gif">
        	<script language="javascript">
             CreateCategorySelection("brandid",0,"--选择商品分类--",null);
          </script>
        	
        	</td>
        <td height="25" background="images/topbg.gif"><input type="button" value="添加热销品牌" onclick="SetHotBrand(this.form)"><input type="hidden" name="recommend" value="<?php echo $maxrecommend+1;?>"></td>
      </tr>		 
     </table></form>
