...
function do_sort($selec,$index){
  global $maxorder;
  $res=$GLOBALS['conn']->query("select * from mg_help where parent=$selec and property=1 order by sortorder");
  foreach($res as $row){
    if($selec==0){
      if($row['sortorder']>$maxorder) $maxorder=$row['sortorder'];?>
    <tr bgcolor="#f7f7f7" height=25>
    <td background="images/topbg.gif" bgcolor="#f7f7f7">&nbsp;&nbsp;<a href="mg_addhelp.php?id=<?php echo $row['id'];?>&action=edit"><b><?php echo $row['sortorder'].'.&nbsp;'.$row['title'];?></b></a></td>
    ...
    <td><?php echo str_repeat('　',$index*2);?><a href="mg_addhelp.php?id=<?php echo $row['id'];?>&action=edit"><?php echo $row['sortorder'].'.&nbsp;'.$row['title'];?></a></td>
...
栏目排序：<input name="sortorder" type="text" class="input_sr" value="<?php echo $maxorder+1;?>" size="3">
