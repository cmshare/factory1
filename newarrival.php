...
$LimitDays=6; 
#当前库房到货情况MAIN_DEPOT
#$sql='select `mg_product`.id,`mg_product`.name,`mg_product`.price3,`mg_category`.sortindex,mg_orders.actiontime,unix_timestamp()-mg_orders.actiontime as sorttime from (((`mg_ordergoods` inner join `mg_orders` on `mg_ordergoods`.ordername=`mg_orders`.ordername) inner join `mg_product` on `mg_ordergoods`.productid=`mg_product`.id)left join `mg_category` on `mg_product`.brand=`mg_category`.id) where (mg_orders.state<-3 and mg_orders.importer='.MAIN_DEPOT.') and `mg_orders`.actiontime>unix_timestamp()-'.$LimitDays.'*24*60*60 union all select `mg_product`.id,`mg_product`.name,`mg_product`.price3,`mg_category`.sortindex,`mg_stocklog`.actiontime,unix_timestamp()-`mg_stocklog`.actiontime as sorttime from ((`mg_stocklog` inner join `mg_product` on `mg_stocklog`.productid=`mg_product`.id) left join `mg_category` on `mg_product`.brand=`mg_category`.id) where `mg_stocklog`.actiontime >unix_timestamp()-'.$LimitDays.'*24*60*60 and `mg_stocklog`.amount>0 and `mg_stocklog`.depot='.MAIN_DEPOT.' order by sorttime,sortindex,name';
