...
if($cid==0){
    $LinkSortGuider='&nbsp;&gt;&gt;&nbsp;热销品牌';
    $query=$conn->query('select id from `mg_category` where recommend>1',PDO::FETCH_NUM);
    foreach($query as $rs){
      if($catsel) $catsel.=' or ';
  ...
  while($PID){
    $row=$conn->query('select id,title,pid from `mg_category` where id='.$PID,PDO::FETCH_ASSOC)->fetch();
      ...
      if($PID==@$UnfoldBrand) $ParentBrand=$row['pid'];
      $PID = $row['pid'];
...
function ShowWareList(){
  global $conn,$mode,$catsel,$LinkSortGuider;
  if($mode==0){
